...
function Map:setObjectData(layer)
	for _, object in ipairs(layer.objects) do
		-- JAILERS CHANGE: Used to use objects.id as index, but I
		-- don't know if that still exists in this tiled version.
...
function Map:setObjectCoordinates(layer)
	...
	for _, object in ipairs(layer.objects) do
		...
		-- JAILERS CHANGE: r has always been undefined in maps I've made
		local cos = r and math.cos(math.rad(r)) or 0
